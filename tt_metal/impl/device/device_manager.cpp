...
#include "firmware/dispatch_firmware_initializer.hpp"
...
void DeviceManager::initialize_fabric_and_dispatch_fw() {
    ...
    initializers_[DispatchFirmwareInitializer::key()]->configure();
...
bool DeviceManager::is_dispatch_firmware_active() const {
    auto it = initializers_.find(DispatchFirmwareInitializer::key());
...
bool DeviceManager::close_devices(const std::vector<IDevice*>& devices, bool /*skip_synchronize*/) {
    ...
    // Teardown all initializers in reverse key order (Dispatch -> Fabric -> Profiler -> CQ)
    for (auto& initializer : std::ranges::reverse_view(initializers_)) {
        initializer.second->teardown();
    }
    initializers_.clear();
