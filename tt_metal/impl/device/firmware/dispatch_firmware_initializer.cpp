...
#include "dispatch_firmware_initializer.hpp"
...
void DispatchFirmwareInitializer::init(const std::vector<IDevice*>& devices) {
    devices_ = devices;
...
void DispatchFirmwareInitializer::configure() {
    if (!using_fast_dispatch()) {
        initialized_ = true;
        return;
    }
...
void DispatchFirmwareInitializer::teardown() {
    if (!descriptor_->is_mock_device() && using_fast_dispatch()) {
...
bool DispatchFirmwareInitializer::is_initialized() const { return initialized_; }

void DispatchFirmwareInitializer::compile_dispatch_kernels() {
...
void DispatchFirmwareInitializer::init_device_command_queues() {
...
void DispatchFirmwareInitializer::terminate_command_queues() {
    // Migrated from DeviceManager::teardown_fd()
    // command_queue() is on the concrete Device class, not IDevice.
    // At this point we know these are real (non-mock) devices.
    for (auto* dev : devices_) {
...
void DispatchFirmwareInitializer::wait_for_dispatch_cores() const {
    for (auto* dev : devices_) {
...
void DispatchFirmwareInitializer::process_termination_signals() const {
    for (auto* dev : devices_) {
...
bool DispatchFirmwareInitializer::using_fast_dispatch() const { return rtoptions_.get_fast_dispatch(); }
