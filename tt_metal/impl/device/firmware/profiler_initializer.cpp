...
void ProfilerInitializer::teardown() {
    ...
    for (auto* dev : devices_) {
        detail::ReadDeviceProfilerResults(dev, ProfilerReadState::ONLY_DISPATCH_CORES);
    }
    detail::ProfilerSync(ProfilerSyncState::CLOSE_DEVICE);

    if (profiler_state_manager_ && getDeviceProfilerState()) {
        // Device profiling data is dumped here instead of MetalContext::teardown() because
        // MetalContext::teardown() is called as a std::atexit() function, and
        // ProfilerStateManager::cleanup_device_profilers() cannot be safely called from a
        // std::atexit() function because it creates new threads, which is unsafe during program
        // termination.
