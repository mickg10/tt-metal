...
void FabricFirmwareInitializer::configure() {
    ...
    log_trace(tt::LogMetal, "Fabric firmware configured");
    initialized_ = true;
}
...
void FabricFirmwareInitializer::compile_and_configure_fabric() {
    std::vector<std::shared_future<IDevice*>> events;
    ...
    for (auto* dev : devices_) {
        events.emplace_back(detail::async([dev]() {
            ...
            return static_cast<IDevice*>(nullptr);
        }));
    }
    ...
    // Sequentially configure fabric on all devices.
    ...
    for (auto* dev : devices_) {
        ...
        for (const auto& tunnel : tunnels_from_mmio) {
            for (auto j = tunnel.size() - 1; j > 0; j--) {
                ...
                auto it =
                    std::find_if(devices_.begin(), devices_.end(), [&](IDevice* d) { return d->id() == tunnel[j]; });
                if (it != devices_.end()) {
