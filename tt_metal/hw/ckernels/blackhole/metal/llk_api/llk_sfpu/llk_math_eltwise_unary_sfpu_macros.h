...
// For ops where init takes multiple template parameters (e.g., approximate, fast_approx, scale, clamp_negative_input).
#define SFPU_TEMPLATE_INIT_KERNEL(OP, INIT_CB, APPROX, FAST_APPROX, SCALE, ...) \
    llk_math_eltwise_unary_sfpu_init<SfpuType::OP, APPROX>(                     \
        INIT_CB<APPROX, FAST_APPROX, SCALE __VA_OPT__(, ) __VA_ARGS__>)
...
#define SFPU_UNARY_KERNEL_THREE_TEMPLATE_ARGS_FN(FN, APPROXIMATE, V, T, DST_IDX) \
    _llk_math_eltwise_unary_sfpu_params_<APPROXIMATE>(                           \
        ckernel::sfpu::FN<V, T>, DST_IDX, static_cast<int>(VectorMode::RC));
