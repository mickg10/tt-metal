...
def test_binary(device):
    ...
    ops_chain = [
        ...
        ttnn.UnaryWithParam(ttnn.UnaryOpType.TYPECAST, 1, 0),
    ]
...
def test_binary_w_typecast(input_shapes, in_dtype, out_dtype, layout, ttnn_fn, device):
    ...
    out_tt = ttnn_op(input_tensor_a, input_tensor_b, dtype=out_dtype, use_legacy=False)

    output_tensor = ttnn.to_torch(out_tt)

    golden_fn = ttnn.get_golden_function(ttnn_op)
    torch_output_tensor = golden_fn(torch_input_tensor_a, torch_input_tensor_b).to(torch.bfloat16)
