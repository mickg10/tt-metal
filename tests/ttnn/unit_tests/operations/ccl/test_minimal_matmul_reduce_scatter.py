...
def run_reduce_scatter_galaxy_impl(
    mesh_device,
    num_devices,
    mesh_shape,
    rs_input_shape,
    ...
    # Use ShardTensor2dMesh for Galaxy
    weight_tt = ttnn.from_torch(
        weights_tensor_padded,
        dtype=matmul_weights_dtype,
        layout=layout,
    ...
    for i in range(num_iters):
        ...
        # Use ShardTensor2dMesh for Galaxy input
        input_tensor_mesh = ttnn.from_torch(
            mm_input_tensor,
            device=mesh_device,
            layout=layout,
    ...
    for i in range(num_iters):
        ...
        # Use ConcatMesh2dToTensor for Galaxy
        tt_mm_out = ttnn.from_device(tt_mm_out_tensor)
        tt_mm_out = ttnn.to_torch(
            tt_mm_out, mesh_composer=ttnn.ConcatMesh2dToTensor(mesh_device, dims=(1, 3), mesh_shape=mesh_shape)
        )
        tt_mm_out = torch.sum(torch.stack(torch.chunk(tt_mm_out, num_devices, 3)), dim=0)
        eq, output = comp_pcc(tt_mm_out, torch_mm_out_tensor)
        logger.info(f"Matmul {output}, shape = {tt_mm_out.shape}, iteration {i}")
        ...
        tt_rs_out = ttnn.from_device(tt_rs_out_tensor)
        tt_rs_out = ttnn.to_torch(
            tt_rs_out, mesh_composer=ttnn.ConcatMesh2dToTensor(mesh_device, dims=(1, 3), mesh_shape=mesh_shape)
        )
        eq, output = comp_pcc(tt_rs_out, torch_rs_out)
        logger.info(f"Reduce Scatter {output}, shape = {tt_rs_out.shape}, iteration {i}")
        if not eq:
...
def test_reduce_scatter_async_galaxy(
    mesh_device,
    num_devices,
    mesh_shape,
    num_links,
    ...
    run_reduce_scatter_galaxy_impl(
        mesh_device,
        num_devices,
        mesh_shape,
        rs_input_shape,
