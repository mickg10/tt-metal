...
ALWI void reduce_tile_to_cb(uint32_t icb0, uint32_t icb_ones, uint32_t ocb, uint32_t size) {
    ...
    tile_regs_wait();
    pack_tile(DST_REG_ID, ocb);
    tile_regs_release();

    cb_push_back(ocb, ONE_TILE);
}
...
ALWI void elementwise_multiply(
    ...
    tile_regs_wait();
    pack_tile(DST_REG_ID, out_cb_id);
    tile_regs_release();
...
ALWI void accumulate(uint32_t accum_cb_id, uint32_t addend_cb_id) {
    ...
    cb_reserve_back(accum_cb_id, ONE_TILE);
    pack_tile(DST_REG_ID, accum_cb_id);
    tile_regs_release();
    cb_push_back(accum_cb_id, ONE_TILE);
}
