...
#include "softmax_backward_nanobind.hpp"
...
void bind_normalization_softmax_backward_operation(nb::module_& mod) {
    ...
    ttnn::bind_function<"softmax_backward">(
        mod,
        doc,
        ttnn::overload_t(
            nb::overload_cast<const ttnn::Tensor&, const ttnn::Tensor&, int32_t>(
                &ttnn::operations::normalization::softmax_backward),
            nb::arg("softmax_output_tensor").noconvert(),
            nb::arg("grad_tensor").noconvert(),
            nb::arg("dim") = -1));
}

void bind_normalization_softmax_backward(nb::module_& mod) { bind_normalization_softmax_backward_operation(mod); }
}  // namespace ttnn::operations::normalization::detail
