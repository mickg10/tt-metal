    ...
    def read_decode_output(self, tt_out, async_read=False):
        if not async_read:
            ...
            if isinstance(tt_out, list):
                tt_out = tt_out[0]
            result = tt_out.cpu()
            # Debug: print decode output for first few steps
            decode_step = getattr(self, "_decode_step_count", 0)
            if decode_step <= 5:
                try:
                    result_torch = ttnn.to_torch(ttnn.get_device_tensors(result)[0])
                    # Check if this is logits (large last dim) or token (small)
                    if result_torch.shape[-1] > 1000:
                        # Logits - print top-5
                        logits_flat = result_torch.float().flatten()[: self.model.vocab_size]
                        topk_vals, topk_ids = torch.topk(logits_flat, k=5)
                        logger.info(
                            "[DECODE_OUTPUT_DEBUG] step={} logits_shape={} top5_ids={} top5_vals={}",
                            decode_step,
                            result_torch.shape,
                            topk_ids.tolist(),
                            topk_vals.tolist(),
                        ...
                        # Token
                        token = int(result_torch.flatten()[0].item())
                        logger.info("[DECODE_OUTPUT_DEBUG] step={} token={}", decode_step, token)
                except Exception as e:
                    logger.info("[DECODE_OUTPUT_DEBUG] step={} failed to debug: {}", decode_step, e)
            return result
