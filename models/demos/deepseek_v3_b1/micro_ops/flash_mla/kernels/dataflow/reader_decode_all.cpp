                ...
                *k_write_ptr_shared = k_write_ptr;
                ...
                if (is_mcast_sender) {
                    ...
                    if constexpr (k_args.is_sharded) {
                        ...
                        // Set NOC state for THIS chunk's shard address
                        // Must be done per-chunk since each chunk may have different DRAM offset
                        ...
                        // Get NOC address for first tile to verify addressing
                        uint64_t first_tile_noc_addr = k_reader.get_noc_addr(k_tile_id);
