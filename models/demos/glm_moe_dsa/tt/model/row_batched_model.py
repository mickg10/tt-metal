...
from models.demos.glm_moe_dsa.utils.config_dataclass import KvCacheConfig, ReshardConfig
...
from models.demos.glm_moe_dsa.utils.shared_state_addon import SharedStateAddOn
...
class RowBatchedModel(SharedStateAddOn, AbstractModule):
    ...
    def forward_decode(
        ...
        if return_hidden:
            ...
            # Do NOT deallocate x — hidden_for_mtp references it and will be used by MTP predict
            logits = LMHead1D.forward_decode(lm_head_in, cfg["lm_head"])
            ttnn.deallocate(lm_head_in)
    ...
    def forward_prefill(
        ...
        if return_hidden:
            lm_head_in = ttnn.clone(x)
            # Do NOT deallocate x — hidden_for_mtp references it for MTP predict
