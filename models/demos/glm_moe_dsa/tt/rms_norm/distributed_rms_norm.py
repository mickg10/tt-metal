...
from models.demos.glm_moe_dsa.tt.rms_norm.rms_norm_base import RMSNormBase
...
class DistributedRMSNorm(RMSNormBase):
    ...
    def decode_model_config(
        ...
        """
        # Grid must divide hidden_size/TP evenly
        if hf_config.hidden_size == 7168:
            shard_core_grid = ttnn.CoreGrid(x=8, y=3)  # 24 cores for hidden=6144  # 28 cores, 7168/4/28=64
        else:
            # GLM-5.1: hidden=6144, TP=4 → 1536/device. 24 cores: 1536/24=64
            shard_core_grid = ttnn.CoreGrid(x=8, y=3)
